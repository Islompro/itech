...
function CollectionDash() {
    ...
    useEffect(() => {
        let data = collection(db, 'collection')
    ...
    const database = collection(db, "collection")
    ...
    const cartDelete = async (id) => {
        const deletePost = doc(db, "collection", id);
        await deleteDoc(deletePost);
        console.log(id);
    }

    const cartEdit = async (img, img1, img2, category, price, matn, title, id) => {
        setImg(img);
        setImg1(img1);
        setImg2(img2);
    ...
    const cartUpdate = async () => {
        const updateData = doc(db, 'collection', id);
        await updateDoc(updateData, { img, img1, img2, category, price, matn, title });
            ...
            <section className="text-gray-600 body-font relative">
                <div className="container px-5 py-24 mx-auto">
                    <div className="flex flex-col text-center w-full mb-12">
                        <h1 className="sm:text-3xl text-2xl font-medium title-font mb-4 text-gray-900">To'plamlar</h1>
                    </div>
            ...
            <div>
                <div className="lg:w-2/3 w-full mx-auto overflow-auto">
                    <div className="flex flex-col">
                        ...
                        <section className="text-gray-600 body-font flex flex-wrap max-w-[1200px] m-auto max-sm:mx-1">
                            {
                                blog.map((data) => {
                                    return (
                                        <Fragment >
                                            <div key={data.id} className="mt-3 w-[100%] flex justify-between items-center mb-3">
                                                <img className="w-[10%] h-[70%]" name="img" id="img" src={data.img} alt="" />
                                                <b className="w-[10%] ml-[-10%] text-center  max-sm:text-[10px] max-sm:ml-[-2%]" name="nameprodust" id="nameprodust">{data.title}</b>
                                                <p className="text-[20px] ml-[-5%] max-sm:text-[12px] max-sm:ml-[2%]" name="price" id="price">{data.price} $</p>
                                                ...
                                                <FiEdit className="text-[25px] text-blue-500 w-16 text-center cursor-pointer" onClick={() => cartEdit(data.img, data.img1, data.img2, data.category, data.price, data.matn, data.title, data.id)} />
